...
class V2Vchannels:
    ...
    def get_path_loss(self, position_A, position_B):
        ...
        CH =0.8 + (1.1 * (np.log10(self.fc * 10^3)-0.7)*self.h_ms)-1.56 * np.log10(self.fc) 

        return 69.55 + 26.16*np.log10(self.fc * 10 ^3)- 13.82 * np.log10(self.h_bs) - CH + (44.9-6.55*np.log10(self.h_bs)* np.log10(distance)) # + self.shadow_std * np.random.normal()

    def get_shadowing(self, delta_distance, shadowing):
        # It use a deterministic term and a random term
        ...
        return math.sqrt(1 - np.exp(-2 * (delta_distance / self.decorrelation_distance))) * np.random.normal(0, 3) # standard dev is 3 db
...
class V2Ichannels:
    ...
    def get_shadowing(self, delta_distance, shadowing):
        ...
        return np.sqrt(1 - np.exp(-2 * (delta_distance / self.Decorrelation_distance))) * np.random.normal(0, 8, nVeh)
...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.time_slow = 0.1  # update slow fading/vehicle position every 100 ms. This is one time step. 
    ...
    # add new vehicles by position, direction and velocity
    ...
    def add_new_vehicles_by_number(self, n):
        ...
        # The distance traveled by vehicles in a given time step
