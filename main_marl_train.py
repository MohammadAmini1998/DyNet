...
# This function give us the observation of each agent
...
agents = []
sesses = []
# Number of V2V links which are n_veh * n_neighbor
...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            ...
            # Here we get the state and action for each agent 
            for i in range(n_veh):
                for j in range(n_neighbor):
                    state = get_state(env, [i, j], i_episode/(n_episode-1), epsilon)
                    state_old_all.append(state)
                    action = predict(sesses[i*n_neighbor+j], state, epsilon)
            ...
            for i in range(n_veh):
                for j in range(n_neighbor):
                    ...
                    state_new = get_state(env, [i, j], i_episode/(n_episode-1), epsilon)
