...
class WeightGeneratorNetwork:
    def __init__(self, sess, n_player, obs_dim):
        ...
        with tf.compat.v1.variable_scope('schedule'):
            self.schedule_policy = self.generate_wg(self.obs_ph, trainable=True)

        with tf.compat.v1.variable_scope('slow_target_schedule'):
            self.target_schedule_policy = tf.stop_gradient(self.generate_wg(self.obs_ph, trainable=False))

        self.actor_vars = tf.compat.v1.get_collection(tf.compat.v1.trainable_variables, scope='schedule')
        ...
        slow_target_sch_vars = tf.compat.v1.get_collection(tf.compat.v1.trainable_variables, scope='slow_target_schedule')
    ...
    def generate_wg_network(self, obs, trainable=True):
        ...
        schedule =tf.keras.layers.Dense(units=1, activation=tf.nn.sigmoid, trainable=trainable)(hidden_3)
