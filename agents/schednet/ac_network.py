...
class CriticNetwork:
    def __init__(self, sess, n_agent, state_dim, nn_id=None):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            self.q_values, self.sch_q_values, self.sch_q_values1 = self.generate_critic_network(self.state_ph, self.priority_ph,self.priority_ph1, trainable=True)
        ...
        with tf.compat.v1.variable_scope('slow_target_'+scope):
            slow_q_values = self.generate_critic_network(self.next_state_ph, self.next_priority_ph,self.next_priority_ph1, trainable=False)
            ...
            self.slow_sch_q_values1 = tf.stop_gradient(slow_q_values[2])
    ...
    def training_critic(self, state_ph, reward_ph, next_state_ph, priority_ph, next_priority_ph,priority_ph1, next_priority_ph1, is_not_terminal_ph):
        ...
        return self.sess.run([self.td_errors, self.critic_train_op],
                             feed_dict={self.state_ph: state_ph,
                                        ...
                                        self.next_priority_ph1: next_priority_ph1,
                                        self.is_training_ph: True})
