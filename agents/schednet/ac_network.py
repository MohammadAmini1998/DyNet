...
class ActionSelectorNetwork:
    ...
    def __init__(self, sess, n_agent, obs_dim_per_unit, action_dim, nn_id=None):
        ...
        # placeholders
        self.state_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 24])
        self.next_state_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, obs_dim_per_unit])
        # Concat action space
    ...
    def generate_actor_network(self, obs, trainable, share=False):
        ...
        ret = comm.generate_comm_network(obs_list, self.obs_dim_per_unit, self.action_dim, self.n_agent)
