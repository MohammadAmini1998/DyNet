...
class ActionSelectorNetwork:
    ...
    def __init__(self, sess, n_agent, obs_dim_per_unit, action_dim, nn_id=None):
        ...
        self.bandwidht_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.n_agent*self.capacity])
        ...
        with tf.compat.v1.variable_scope(scope):
            self.actions = self.generate_actor_network(self.state_ph, self.schedule_ph,self.bandwidht_ph, trainable=True)
    ...
    def generate_actor_network(self, obs, schedule,bandwidth, trainable, share=False):

        obs_list = list()
        for i in range(self.n_agent):
            obs_list.append(obs[:, i * self.obs_dim_per_unit:(i + 1) * self.obs_dim_per_unit])

        ret = comm.generate_comm_network(obs_list, self.obs_dim_per_unit, self.action_dim, self.n_agent,bandwidth, schedule=schedule)
    ...
    def training_actor(self, state_ph, action_ph, schedule_ph,bandwidth, td_errors):
        return self.sess.run(self.actor_train_op,
                             feed_dict={self.state_ph: state_ph,
                                        self.action_ph: action_ph,
                                        self.schedule_ph: schedule_ph,
                                        self.bandwidht_ph:bandwidth,
...
class CriticNetwork:
    def __init__(self, sess, n_agent, state_dim, nn_id=None):
        ...
        # placeholders
        self.state_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, state_dim])
        self.reward_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None])
        self.next_state_ph = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, state_dim])
